            ...
            for x, y in train_bar:  
                ...
                try:
                    ...
                    if "out of memory" in str(exception):
                        ...
                        raise exception
                D_loss = discriminator_loss(D(fft2(OR_img)), D(SR_Freq))
                ...
                try: 
                    ...
                    if "out of memory" in str(exception):
                        ...
                        raise exception
                logits_SR, logits_OR = D(SR_Freq), D(OR_Freq)
                G_loss = generator_loss(logits_SR, OR_Freq, SR_Freq, lambd)
                G_loss.backward()
                # # Changes invoked by Apex.amp
                ...
                running_results['psnr'] += calculate_psnr(SR_img.data.cpu().numpy(), torch.squeeze(OR_img).data.cpu().numpy()) * BATCH_SIZE
