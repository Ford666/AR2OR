...
class ResBlock(nn.Module):
    def __init__(self, channals_out, channals_m, channals_in, Res_bool):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.GroupNorm(int(channals_m/32), channals_m),
            nn.LeakyReLU(0.1)
            )
        ...
        self.conv3 = nn.Sequential(
            ...
            nn.GroupNorm(int(channals_out/32), channals_out),
            )
...
class down(nn.Module):
    def __init__(self, in_ch, m_ch, out_ch, kernel_size=4, padding=1, stride=2):
        ...
        self.d = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
...
class up(nn.Module):
    def __init__(self, in_ch, m_ch, out_ch, kernel_size=4, padding=1, stride=2):
        ...
        self.u = nn.Upsample(scale_factor=(2, 2), mode="bilinear", align_corners=True)
...
class down_conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1):
        ...
        self.cnn=nn.Sequential(
            ...
            nn.GroupNorm(int(out_ch/32), out_ch),
            nn.LeakyReLU(0.1)
            )
    def forward(self, x):
        return self.cnn(x)
...
def FMAE_loss(input, target): 
    ...
    MAE_fn = torch.nn.L1Loss(reduction='mean')
    [N,C,H,W] = input.size()
    Finput, Ftarget = torch.zeros(N,H,W).type(datype), torch.zeros(N,H,W).type(datype)
    for i in range(N):
        RealFinput, RealFtarget = torch.rfft(input[i,0,:,:],2,False,False), \
                            torch.rfft(target[i,0,:,:],2,False,False)
        Finput[i,:,:], Ftarget[i,:,:] = RealFinput[:,:,0], RealFtarget[:,:,0]

    Ferror = MAE_fn(Finput, Ftarget)
    return Ferror
...
def generator_loss(logits_fake, real_data, fake_images, lambda1):
    """
    ...
    #tv = TV_loss(fake_images)  #1e-2*mse/tv
    fmse = 0 #FMAE_loss(fake_images, real_data)
    lambda2 = (mse+lambda1*fmse)/(4*SquCE)
    return mse+lambda1*fmse+lambda2*SquCE 