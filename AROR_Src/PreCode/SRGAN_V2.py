...
class Generator(nn.Module):
    ...
    def forward(self, x):
        x_s1_skip, x_s1_down = self.s1_down(x.view(-1, 1, 384, 384))
...
def generator_loss(logits_fake, real_data, fake_images):
    ...
    """
    lambda1, lambda2 = 5e-3, 5e-2
    label = torch.ones_like(logits_fake).to(logits_fake.device)
    bce = bce_loss(logits_fake, label) 
    mse = MSE_loss(fake_images, real_data) 
    npcc = -((1+PCC_loss(fake_images, real_data))/2).log()
    return lambda1*bce + mse + lambda2*npcc


def tine_G_loss(logits_fake, real_data, fake_images):
    """
    Computes the generator loss: BCE+MSE+SSIM
    ...
    return bce + lambda1*mse + lambda2*ssim