...
if __name__ == "__main__":
    ...
    # Vertify model
    data, target = next(enumerate(train_loader))[-1][0], next(enumerate(train_loader))[-1][1] #(BATCH_SIZE,384,384)
    data, target = (data.unsqueeze(1)).type(dtype), (target.unsqueeze(1)).type(dtype)
    show_images(data, '../demo_AR.png')
    ...
    optimizerG, optimizerD = torch.optim.Adam(G.parameters(), lr=1e-3, betas=(0.5, 0.999)), \
                torch.optim.Adam(D.parameters(), lr=1e-4, betas=(0.5, 0.999))
...
D(x): %.4f D(G(z)): %.4f  PSNR: %.4f' % (epoch, NUM_EPOCHS, iter_count, iter_per_epoch, 
        ...
        D_path, G_path = "../datasplit/new_train/D_model%d.pkl" % epoch, "../datasplit/new_train/G_model%d.pkl" % epoch
        torch.save(D.state_dict(), D_path)  
        torch.save(G.state_dict(), G_path)                 
        print("SRGAN model saved in %dth epoch!\n" % epoch)

        # save train_result per epoch
        stat_path = '../SRGAN_train_result.csv'
        data_frame = pd.DataFrame(
            data = {'Iters':train_result['iter'], 'Loss_D':train_result['d_loss'],
                    'Loss_G':train_result['g_loss'], 'PSNR':train_result['psnr']})
