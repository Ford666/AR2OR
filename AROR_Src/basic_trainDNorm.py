...
if __name__ == "__main__":
    ...
    RecFile = open('../datasplit/new_train/train8/RecordDNorm.txt', 'w+')
    ...
    Norms = list(range(1,5))
    Normstrs = ['None','BN','IN','GN','LN']  
    ...
    for norm in Norms:
        # Import SRGAN model
        G = Generator().cuda()
        D = Discriminator_WGAN((2,1,384,384),norm).cuda()
        ...
        print("Training started, Norm=%s" % Norm)
        RecFile.write("Training started, Norm=%s\n" % Norm)
        RecFile.flush()
        ...
        SR_path, AR_path, OR_path = train_path+'SR_iter/Norm=%s/' % Norm, \
                            train_path+'AR_iter/Norm=%s/' % Norm, train_path+'OR_iter/Norm=%s/' % Norm
        mkdir(SR_path)
        mkdir(AR_path)
        mkdir(OR_path)
        ...
        for epoch in range(1, 1+NUM_EPOCHS): #NUM_EPOCHS             
            for x, y in train_loader:
                ...
                WDfake = D(SR_img)
                G_loss, MAE, FMAE, LossG_adv = generator_loss(WDfake, OR_img, SR_img, 1)
...
d_loss, g_loss, l1_loss, d_score, g_score, psnr))
...
d_loss, g_loss, l1_loss, d_score, g_score, psnr))
        ...
        ax1.plot(np.arange(1,Recordlen+1),Loss_Gs, label="Norm=%s" % Norm)
        ax2.plot(np.arange(1,Recordlen+1), L1Loss, label="Norm=%s" % Norm)
        ax3.plot(np.arange(1,Recordlen+1),Loss_Ds, label="Norm=%s" % Norm)
        ax4.plot(np.arange(1,Recordlen+1), Dxs, label="Norm=%s" % Norm)
        ax5.plot(np.arange(1,Recordlen+1), DGzs, label="Norm=%s" % Norm)
        ax6.plot(np.arange(1,Recordlen+1), PSNRs, label="Norm=%s" % Norm)
