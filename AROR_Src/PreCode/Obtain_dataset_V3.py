...
'''
Given AR/OR aug of size (8,144,2000,2000), through 144 different combinations of 5 transforms
...
AR_train = np.zeros(shape=(6*144*100,256,256), dtype=np.float16)
OR_train = np.zeros(shape=(6*144*100,256,256), dtype=np.float16)
...
for i in range(8):
    ...
    if i == 0:    # Val set, 1st pair of images sized (2000,1427)
        ...
        for j in range(144):
            ...
            for k in range(data_len):
                val_lookup.append({'NO.': val_sum-data_len+k, 'Origin': i+1})                
                val_lookup[val_sum-data_len+k].update(parms[j])
                val_lookup[val_sum-data_len+k]['Coord'] = mixed_data[1][k]
        ...
        for j in range(144):           
            ...
            for k in range(data_len):
                test_lookup.append({'NO.':test_sum-data_len+k, 'Origin': i+1})
                test_lookup[test_sum-data_len+k].update(parms[j])
                test_lookup[test_sum-data_len+k]['Coord'] = mixed_data[1][k]
        ...
        for j in range(144):
            f.write(str(i+1) + '|' + str(j+1) + '\n')
            f.flush()
            if i == 2: # 3rd pair of images sized (1500,1500)
                mixed_data = Extract_Patch(AR_aug[i,j,:,:], OR_aug[i,j,:,:], 1500, 1500, 256, 256, 80) # 8*8 patches
            else:
                mixed_data = Extract_Patch(AR_aug[i,j,:,:], OR_aug[i,j,:,:], 2000, 2000, 256, 256, 64)
            data_len = mixed_data[0]
            data_coord[i].append(mixed_data[1])
            ...
            for k in range(data_len):
                train_lookup.append({'NO.':train_sum-data_len+k, 'Origin': i+1})
...
np.save('../Data_aug/AR_train.npy', AR_train[0:train_sum])
np.save('../Data_aug/AR_val.npy', AR_val[0:val_sum])
np.save('../Data_aug/AR_test.npy', AR_test[0:test_sum])

np.save('../Data_aug/OR_train.npy', OR_train[0:train_sum])
np.save('../Data_aug/OR_val.npy', OR_val[0:val_sum])
np.save('../Data_aug/OR_test.npy', OR_test[0:test_sum])
