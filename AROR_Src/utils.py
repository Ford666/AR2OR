...
def show_images(images, path):
    images = np.squeeze(images.data.cpu().numpy()) #(N,H,W) ndarray
    (N,H,W) = images.shape
    #im mode, Ref to https://www.zhihu.com/question/68390406 DON'T use 'L' or 'P'
    JoImg = Image.new('F', (W*N,H))  
    
    
    for i, im in enumerate(images):
        # im_min, im_max = np.amin(np.amin(im,0),0), np.amax(np.amax(im,0),0)
        # im = (im-im_min)/(im_max-im_min)
        # im = np.rint(255*im).astype(np.uint16)
        #np. asarray() <--> Image.fromarray() 
        JoImg.paste(Image.fromarray(im), (i*W,0,(i+1)*W,H))  #box为2元数组（左上角坐标）或4元数组（左上右下）
...
def stitch_SavePatch(SR_path, patchs, rows, cols, patchH, patchW, overlap):  # patch_path,
    ...
    ImgMean, ImgStd = np.mean(imgs), np.std(imgs, ddof=1)
    imgs[imgs<0] = 0
    plt.imsave(SR_path+'.png', np.power(imgs,0.8), cmap=cm.hot) # cmap=cm.hot, vmin=0, vmax=ImgMean-ImgStd
    return imgs
        ...
        for j in range(cols):
            if (img[i,j]>threshold1):
                if i-edge>=0 and i+edge<rows and j-edge>=0 and j+edge<cols:
                    ...
                    # if TempStd>1.5*std and ncount<2/3*Templen:    
                    #     img_denoise[i, j] = 0  #img_denoise[i-edge:i+edge+1, j-edge:j+edge+1]=0 
                    
    return img_denoise

def DenoiseHist(img):
    #Remove extremly bright pixels
    rows, cols = img.shape
    img_denoise = img.copy()
    Mean, Std = np.mean(img.reshape(-1,1)), np.std(img.reshape(-1,1))
    thro = Mean+4*Std
    print("Expected Img Thro: %.4f" % thro)
    # plt.hist(img)
    # plt.show()
    img_denoise = img*(img<0.5)
    return img_denoise
...
def Elastic_Deformation(img, sigma, seed):
    ...
    if seed > 40:
        ...
        image = cv2.warpAffine(img,m,(img.shape[1],img.shape[0]),borderMode=cv2.BORDER_REFLECT_101) #random affine transform
