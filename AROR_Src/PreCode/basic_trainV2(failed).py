            ...
            for x, y in train_bar:  
                ...
                try:
                    ...
                    if "out of memory" in str(exception):
                        ...
                        raise exception
                D_loss = discriminator_loss(D(OR_Exp), D(SR_img))
                ...
                try: 
                    ...
                    if "out of memory" in str(exception):
                        ...
                        raise exception
                logits_SR, logits_OR = D(SR_img), D(OR_Exp)
                G_loss = generator_loss(logits_SR, OR_Exp, SR_img, lambd)
                G_loss.backward()
                # # Changes invoked by Apex.amp
                ...
                running_results['psnr'] += calculate_psnr(SR_Inte.data.cpu().numpy(), OR_img.data.cpu().numpy()) * BATCH_SIZE
                ...
                if (iters % ITER_PER_EPOCH <= 100 and iters % 2 == 0) or \
                    ...
                    train_bar.set_description(desc="Lambd: %.6f, Epoch|NUM_EPOCHS: %d/%d, Iter/Iter_num: %d/%d, \
Loss_D:%.6f, Loss_G:%.6f, D(x):%.4f, D(G(z)):%.4f, PSNR:%.4f, G_lr:%.4e, D_lr:%.4e" % (lambd, epoch, NUM_EPOCHS,
iters, NUM_EPOCHS*ITER_PER_EPOCH, d_loss, g_loss, d_score, g_score, psnr, G_lr, D_lr))  
                ...
                if (iters % ITER_PER_EPOCH) % 1000 == 0:
                    ...
                    show_images(SR_Inte, img_path+'%s/SR_iter/'%lambd+str(iters)+'.png')
                    show_images(torch.squeeze(AR_img), img_path+'%s/AR_iter/'%lambd+str(iters)+'.png')
                    show_images(torch.squeeze(OR_img), img_path+'%s/OR_iter/'%lambd+str(iters)+'.png')
            ...
            with torch.no_grad():
                ...
                for x, y in val_loader:
                    AR_img, OR_img = (x.unsqueeze(1)).type(dtype), (y.unsqueeze(1)).type(dtype) #(BATCH_SIZE,1,384,384)
                    SR_img = G(AR_img)
                    SR_img = InteAmpPha(SR_img)
                    SR_imgs = torch.cat((SR_imgs, SR_img),0)

            SR_path = "../datasplit/new_train/train7/SR%d" % epoch
