...
'''
Finetune Loss components for WGAN-GP model
'''
...
if __name__ == "__main__":
    ...
    # Import SRGAN model
    G = Generator().cuda()
    D = Discriminator_WGAN((2,1,384,384),3).cuda()
    ...
    RecFile = open('../datasplit/new_train/train8/RecordAddlossG.txt', 'w+')
    ...
    for opt in opts:
        G_lr_init, D_lr_init = 1e-4, 1e-4
        G_lr_end, D_lr_end = 0, 0
        AddLoss = 'None' if opt==0 else 'FMAE' if opt==1 else 'TV'
        ...
        print("Training started, Addloss=%s" % AddLoss)
        RecFile.write("Training started, Addloss=%s\n" % AddLoss)
        RecFile.flush()
        ...
        SR_path, AR_path, OR_path = train_path+'SR_iter/AddlossG%d/' % opt, \
                            train_path+'AR_iter/AddlossG%d/' % opt, train_path+'OR_iter/AddlossG%d/' % opt
        mkdir(SR_path)
        mkdir(AR_path)
        mkdir(OR_path)
        ...
        for epoch in range(1, 1+NUM_EPOCHS): #NUM_EPOCHS             
            for x, y in train_loader:
                ...
                WDfake = D(SR_img)
                G_loss, MAE, addloss, LossG_adv = generator_loss(WDfake, OR_img, SR_img, opt)
                L1_loss = MAE+1e-2*addloss
                G_loss.backward()
...
d_loss, g_loss, 0.9*(MAE+0.01*addloss), d_score, g_score, psnr))
                    print("Epoch|NUM_EPOCHS: %d/%d, Iter/Iter_num: %d/%d, Loss_D:%.6f, Loss_G:%.6f, \
MAE:%.6f, %s:%.6f, D(x):%.4f, D(G(z)):%.4f, PSNR:%.4f\n" % (epoch, NUM_EPOCHS, iters, NUM_EPOCHS*ITER_PER_EPOCH, \
d_loss, g_loss, MAE, AddLoss, addloss, d_score, g_score, psnr))
                    RecFile.flush()
        ...
        ax1.plot(np.arange(1,Recordlen+1),Loss_Gs, label='Addloss=%s' % AddLoss)
        ax2.plot(np.arange(1,Recordlen+1), L1Loss, label='Addloss=%s' % AddLoss)
        ax3.plot(np.arange(1,Recordlen+1),Loss_Ds, label='Addloss=%s' % AddLoss)
        ax4.plot(np.arange(1,Recordlen+1), Dxs, label='Addloss=%s' % AddLoss)
        ax5.plot(np.arange(1,Recordlen+1), DGzs, label='Addloss=%s' % AddLoss)
        ax6.plot(np.arange(1,Recordlen+1), PSNRs, label='Addloss=%s' % AddLoss)
