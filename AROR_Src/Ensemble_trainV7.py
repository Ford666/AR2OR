...
if __name__ == "__main__":
    ...
    # G.load_state_dict(torch.load('../datasplit/esemble_train/train7/G_model4.pkl'))
    # D.load_state_dict(torch.load('../datasplit/esemble_train/train7/D_model4.pkl'))
    ...
    iter_count = 1+15*25569

    print("Initializing Snapshot Esembles Training!")
    
    for cycle in range(NUM_CYCLES-1,NUM_CYCLES):
        ...
        for epoch in range(EPOCH_PER_CYCLE[cycle]):
...
iters, ITER_PER_CYCLE[cycle], d_loss, g_loss, d_score, g_score, psnr, G_lr, D_lr))
                ...
                if  (iter_count % ITER_PER_EPOCH) % 1000 == 0:
                    img_path = '../datasplit/esemble_train/train7/'
        ...
        SR_path = "../datasplit/test/7th/SRGAN_esemble/test7/SR_cycle%d" % (cycle+1)
        ...
        D_path, G_path = "../datasplit/esemble_train/train7/D_model%d.pkl" % (cycle+1), "../datasplit/esemble_train/train7/G_model%d.pkl" % (cycle+1)
        torch.save(D.state_dict(), D_path)  
        torch.save(G.state_dict(), G_path)                 
        print("Take a snapshot of SRGAN in %dth cycle!\n" % (cycle+1))

        # save train_result per cycle
        stat_path = '../datasplit/esemble_train/train7/SRGAN_esemble_train.csv'
