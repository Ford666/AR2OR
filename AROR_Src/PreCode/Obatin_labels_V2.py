...
transform and adding noise, to get AR/OR labels of same size(10,144,2000,2000) as AR/OR_aug. Then with the recorded coordinates 
...
for i in range(10):
    for j in range(144):
        if i == 2: #only AR/OR_3 with size(1567,1567)
            AR_label[2, j, 0:1567, 0:1567] = Add_Gaussian_Noise(Gamma_Transform(Elastic_Deformation(Rotation(Flip(AR_npy[2,0:1567,0:1567], 
            ...
            OR_label[2, j, 0:1567, 0:1567] = Add_Gaussian_Noise(Gamma_Transform(Elastic_Deformation(Rotation(Flip(OR_npy[2,0:1567,0:1567], 
                          parms_label[j]['flip_option']), parms_label[j]['rot_option']), parms_label[j]['ela_parm']['sigma'],
                           parms_label[j]['ela_parm']['seed']), parms_label[j]['gamma_option']), parms_label[j]['noise_option'])
        else:
...
#f.write ("Read data labels done: AR_label(10,144,2000,2000), OR_label(10,144,2000,2000)" + '\n')
#f.flush()
