...
def Screening_Patch(img, patchH, patchW):     
    ...
    ScreenPatch = []
    patch_num = img.shape[0]//patchH * img.shape[1]//patchW
    x = 0
    for i in range(img.shape[0]//patchH):
        for j in range(img.shape[1]//patchW):
            #export_path1 = '../Vessel_densiy/image_patch/'+str(x+1)+'.png'
            #mpimg.imsave(export_path1, img[i*patchH:(i+1)*patchH,
            #                           j*patchW:(j+1)*patchW])

            density = np.mean(img_remove[i*patchH:(i+1)*patchH,
                                       j*patchW:(j+1)*patchW])
            if density > density_mean/6: # threshold for screening
                #export_path2 = '../Vessel_densiy/screening_patch/'+str(x+1)+'.png'
                #mpimg.imsave(export_path2, img[i*patchH:(i+1)*patchH,
                #                       j*patchW:(j+1)*patchW])
                ScreenPatch.append(img[i*patchH:(i+1)*patchH,
                                       j*patchW:(j+1)*patchW])
            x += 1
    return ScreenPatch.shape[0], np.array(ScreenPatch)

#OR_patch = Screening_Patch(OR_denoise,(256,256))
#print(OR_patch.shape)


#Image patch extraction based on vessel density sceening result
def Extract_Patch(img1, img2, patchH, patchW, overlap, rand_row):
    """
    img: ndarray
    ...
    TrainSet1, ValSet1, TestSet1 = [], [], []
    TrainSet2, ValSet2, TestSet2 = [], [], []
            ...
            if density < density_mean/6: # threshold for screening
                ...
                if (i == rand_row ): 
                    if (j % 2) == 0:   # patches from one random row and its even-numbered columns => Val set
                        ValSet1.append(img1[i*(patchH-overlap):(i+1)*patchH-i*overlap,
                                       j*(patchW-overlap):(j+1)*patchW-j*overlap])
                        ValSet2.append(img2[i*(patchH-overlap):(i+1)*patchH-i*overlap,
                                       j*(patchW-overlap):(j+1)*patchW-j*overlap])

                    else: # patches from one random row and its odd-numbered columns => Test set
                        TestSet1.append(img1[i*(patchH-overlap):(i+1)*patchH-i*overlap,
                                       j*(patchW-overlap):(j+1)*patchW-j*overlap])
                        TestSet2.append(img2[i*(patchH-overlap):(i+1)*patchH-i*overlap,
                                       j*(patchW-overlap):(j+1)*patchW-j*overlap])

                else:
                    TrainSet1.append(img1[i*(patchH-overlap):(i+1)*patchH-i*overlap,
                                       j*(patchW-overlap):(j+1)*patchW-j*overlap])
                    TrainSet2.append(img2[i*(patchH-overlap):(i+1)*patchH-i*overlap,
                                       j*(patchW-overlap):(j+1)*patchW-j*overlap])

    TrainSet1, ValSet1, TestSet1 = np.array(TrainSet1), np.array(ValSet1), np.array(TestSet1)
    TrainSet2, ValSet2, TestSet2 = np.array(TrainSet1), np.array(ValSet1), np.array(TestSet1)

    set_len = [len(TrainSet1),len(ValSet1),len(TestSet1)]
...
def Divide_Dataset(Aug_data):
    train_set = np.zeros(shape=(10*144*90,256,256), dtype=np.float32)
    train_label = np.zeros(shape=(10,144,1), dtype=np.uint8)
    val_set = np.zeros(shape=(10*144*5,256,256), dtype=np.float16)
    val_label = np.zeros(shape=(10,144,1,1), dtype=np.uint8)
    test_set = np.zeros(shape=(10*144*5,256,256), dtype=np.float16)
    test_label = np.zeros(shape=(10,144,1,1), dtype=np.uint8)
    ...
    for i in range(10):
        for j in range(144):
            ...
            mixed_data = Extract_Patch(Aug_data1[i,j,:,:],Aug_data2[i,j,:,:],256,256,7,rand_row)
            ...
            train_label[i,j,:], val_label[i,j,0,:], test_label[i,j,0,:] =  \
...
AR_train = np.zeros(shape=(10*144*90,256,256), dtype=np.float16)
AR_train_label = np.zeros(shape=(10,144,1), dtype=np.uint8)
OR_train = np.zeros(shape=(10*144*90,256,256), dtype=np.float16)
OR_train_label = np.zeros(shape=(10,144,1), dtype=np.uint8)

AR_val = np.zeros(shape=(10*144*5,256,256), dtype=np.float16)
AR_val_label = np.zeros(shape=(10,144,1,1), dtype=np.uint8)
OR_val = np.zeros(shape=(10*144*5,256,256), dtype=np.float16)
OR_val_label = np.zeros(shape=(10,144,1,1), dtype=np.uint8)

AR_test = np.zeros(shape=(10*144*5,256,256), dtype=np.float16)
AR_test_label = np.zeros(shape=(10,144,1,1), dtype=np.uint8)
OR_test = np.zeros(shape=(10*144*5,256,256), dtype=np.float16)
OR_test_label = np.zeros(shape=(10,144,1,1), dtype=np.uint8)
