...
if __name__ == "__main__":
    ...
    for lambda1 in lambda1s: 
        ...
        # G.load_state_dict(torch.load('../datasplit/new_train/train3/G_model1.pkl'))
        # D.load_state_dict(torch.load('../datasplit/new_train/train3/D_model1.pkl'))
        ...
        for epoch in range(1, 1+NUM_EPOCHS): #NUM_EPOCHS
            G.train()
            D.train()
            train_bar = tqdm(train_loader, ncols=230)  #tqdm滚动条不能单行显示，需要人为规定长度；以及在进程异常情况下设置tqdm完全退出 
            for x, y in train_bar:  
                ...
                # G_loss = generator_loss(logits_SR, OR_img, SR_img, lambda1)
                # G_loss.backward()
                # # # Changes invoked by Apex.amp
                ...
                if (iters % ITER_PER_EPOCH <= 100 and iters % 2 == 0) or \
                    ...
                    train_bar.set_description(desc="Lambda1: %.4f, Epoch|NUM_EPOCHS: %d/%d, Iter/Iter_num: %d/%d, \
Loss_D:%.6f, Loss_G:%.6f, D(x):%.4f, D(G(z)):%.4f, PSNR:%.4f, G_lr:%.4e, D_lr:%.4e" % (lambda1, epoch, NUM_EPOCHS,
iters, NUM_EPOCHS*ITER_PER_EPOCH, d_loss, g_loss, d_score, g_score, psnr,G_lr, D_lr))  

                if (iters % ITER_PER_EPOCH) % 1000 == 0:
                    img_path = '../datasplit/new_train/train6/'
                    # show_images(SR_img, img_path+'%s/SR_iter/'%lambda1+str(iters)+'.png')
                    # show_images(AR_img, img_path+'%s/AR_iter/'%lambda1+str(iters)+'.png')
                    # show_images(OR_img, img_path+'%s/OR_iter/'%lambda1+str(iters)+'.png')
            ...
            SR_path = "../datasplit/new_train/train6/SR%d" % epoch
            ...
            D_path, G_path = "../datasplit/new_train/train6/D_model%d.pkl" % epoch, "../datasplit/new_train/train6/G_model%d.pkl" % epoch
            torch.save(D.state_dict(), D_path)  
            torch.save(G.state_dict(), G_path)                 
            print("Save SRGAN model in %dth epoch!\n" % epoch)

            # save train_result per epoch
            stat_path = '../datasplit/new_train/train6/SRGAN_basictrain.csv'
