...
class ResidualDenseBlock(nn.Module):
    def __init__(self, in_ch, out_ch, growth_rate=16, bias=True):
        super(ResidualDenseBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_ch, growth_rate, 3, 1, 1, bias=bias)
        self.bn1 = nn.InstanceNorm2d(growth_rate)
        self.conv2 = nn.Conv2d(in_ch + growth_rate, growth_rate, 3, 1, 1, bias=bias)
        self.bn2 = nn.InstanceNorm2d(in_ch + growth_rate)
        self.conv3 = nn.Conv2d(in_ch + 2 * growth_rate, growth_rate, 3, 1, 1, bias=bias)
        self.bn3 = nn.InstanceNorm2d(in_ch + 2 * growth_rate)
        self.conv4 = nn.Conv2d(in_ch + 3 * growth_rate, growth_rate, 3, 1, 1, bias=bias)
        self.bn4 = nn.InstanceNorm2d(in_ch + 3 * growth_rate)
        self.conv5 = nn.Conv2d(in_ch + 4 * growth_rate, out_ch, 3, 1, 1, bias=bias)
        self.bn5 = nn.InstanceNorm2d(out_ch)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

        self.cnn_skip = nn.Conv2d(in_ch, out_ch, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        return self.lrelu(self.bn5(x5 * 0.2 + x))

class down(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=4, padding=1, stride=2):
        super(down, self).__init__()
        self.same = nn.Sequential(ResidualDenseBlock(in_ch, out_ch))
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=4, padding=1, stride=2):
        ...
        self.h = nn.Sequential(ResidualDenseBlock(2*in_ch, out_ch))
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.s1_down = down(in_ch=1, out_ch=64, kernel_size=4, padding=1, stride=2)#64x256x256
        self.s2_down = down(in_ch=64, out_ch=128, kernel_size=4, padding=1, stride=2)#128x128x128
        self.s3_down = down(in_ch=128, out_ch=256, kernel_size=4, padding=1, stride=2)#256x64x64
        self.s4_down = down(in_ch=256, out_ch=512, kernel_size=4, padding=1, stride=2)#512x32x32
        self.down = down_conv(in_ch=512, out_ch=512, kernel_size=3, stride=1, padding=1)#512x16x16
        self.s4_up = up(in_ch=512, out_ch=256)#256x32x32
        self.s3_up = up(in_ch=256, out_ch=128)#128x64x64
        self.s2_up = up(in_ch=128, out_ch=64)#64x128x128
        self.s1_up = up(in_ch=64, out_ch=32)#32x256x256
...
def discriminator_loss(logits_real, logits_fake): 
    ...
    #real_fake = logits_real - torch.mean(logits_fake)
    #fake_real = logits_fake - torch.mean(logits_real)
    ## Adversarial loss for Relativistic GAN
    #loss = bce_loss(real_fake, real_label) + bce_loss(fake_real, fake_label)
...
def generator_loss(logits_fake, real_data, fake_images):
    ...
    #real_fake = logits_real - torch.mean(logits_fake)
    #fake_real = logits_fake - torch.mean(logits_real   
    #loss = lambda1 * (bce_loss(fake_real, real_label) + bce_loss(real_fake, fake_label)) + \
    #       MSE_loss(fake_images, real_data) - lambda2 * ((1+SSIM_loss(fake_images, real_data))/2).log()

    label = torch.ones_like(logits_fake).to(logits_fake.device)
    loss = bce_loss(logits_fake, label) +  lambda1 * MSE_loss(fake_images, real_data) -\
