...
for i in range(8):
    ...
    if i == 5:  # val set
        ...
        for j in range(288):           
            ...
            mixed_data = Extract_Patch(AR_aug[i,j,:,:], OR_aug[i,j,:,:], 2000, 2000, 384, 384, 64) 
        ...
        for j in range(288):
            ...
            f.flush()
            if i == 0: # 1st pair
                mixed_data = Extract_Patch(AR_aug[i,j,:,:], OR_aug[i,j,:,:], 2000, 1427, 384, 384, 64)  
            elif i == 2: # 3rd pair of images sized (1500,1500)
                mixed_data = Extract_Patch(AR_aug[i,j,:,:], OR_aug[i,j,:,:], 1500, 1500, 384, 384, 80) 
            elif i == 3: # 5th pair
                mixed_data = Extract_Patch(AR_aug[i,j,:,:], OR_aug[i,j,:,:], 1988, 1980, 384, 384, 66)
            else:
                mixed_data = Extract_Patch(AR_aug[i,j,:,:], OR_aug[i,j,:,:], 2000, 2000, 384, 384, 64)
            data_len = mixed_data[0]
            data_coord[i].append(mixed_data[1])
            ...
            for k in range(data_len):
                x_dir, y_dir = '../datasplit/new_train/x/'+str(train_sum-data_len+k+1).zfill(5)+'.npy', \
                                '../datasplit/new_train/y/'+str(train_sum-data_len+k+1).zfill(5)+'.npy'
...
extract_patch(AR_aug[4][0], OR_aug[4][0], 2000, 2000, 384, 384, 64, '../datasplit/test/1/')
extract_patch(AR_aug[4][6], OR_aug[4][6], 2000, 2000, 384, 384, 64, '../datasplit/test/2/')
extract_patch(AR_aug[4][12], OR_aug[4][12], 2000, 2000, 384, 384, 64, '../datasplit/test/3/')
