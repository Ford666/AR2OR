...
# SRGAN_V8 (July 21) 
#(1)It turns out simply concatenating image and its frequency amp & phase is not feasible for Network
#training, due to the distinct data distribution. Therefore, SRGAN_V8 works only for SR in frequency domain.
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.s1_down = down(in_ch=2, m_ch=32, out_ch=64)
        ...
        self.output = nn.Conv2d(in_channels=32, out_channels=2, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        Fx = fft2(x)
        x_s1_skip, x_s1_down = self.s1_down(Fx)
...
def fft2(x):
    (N,_,H,W) = x.size()
    Temp = np.fft.fft2(x.data.cpu().numpy())
    Fx = torch.zeros(N,2,H,W).type(datype)
    Fx[:,0,:,:] = torch.tensor(np.squeeze(Temp.real), requires_grad=True).type(datype)
    Fx[:,1,:,:] = torch.tensor(np.squeeze(Temp.imag), requires_grad=True).type(datype)
    return Fx


def ifft2(Fx):
    (N,_,H,W) = Fx.size()
    x = torch.zeros(N,1,H,W).type(datype)
    Temp = Fx[:,0,:,:].data.cpu().numpy()+1j*Fx[:,1,:,:].data.cpu().numpy()
    x = np.fft.ifft2(Temp)
    x = torch.tensor(np.abs(x), requires_grad=True).type(datype)
    return x
...
def MAE_loss(Input, target): 
    """
     Inputs: 2个channel，CH1频域幅度，CH2频域相位
    - Input: (N, 2, H, W).
    - target: (N, 2, H, W).
...
def generator_loss(logits_fake, real_data, fake_images, lambd):
    ...
    mse = MAE_loss(fake_images, real_data)
    return mse + lambd*SquCE