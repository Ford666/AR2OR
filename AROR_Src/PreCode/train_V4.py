...
if __name__ == "__main__":
    ...
    # G.load_state_dict(torch.load('../datasplit/new_train/G_model6.pkl'))
    # D.load_state_dict(torch.load('../datasplit/new_train/D_model6.pkl'))
          
    
    G_lr_init, G_lr_end = 2e-3, 2.5e-4
    D_lr_init, D_lr_end = 1e-4, 6.25e-6
    ...
    iter_count = 1 #to record iter times in the total training
    
    for cycle in range(1, NUM_CYCLES+1):
     
        running_results = {'batch_sizes': 0, 'd_loss': 0, 'g_loss': 0, 'd_score': 0, 'g_score': 0, 'psnr': 0}
        iters = 1 #to record iter times within a cycle
        G.train()
        D.train()
        ...
        for epoch in range(1, EPOCH_PER_CYCLE+1):
            ...
            for x, y in train_bar:
                ...
                optimizerG.state_dict()["param_groups"][0]["lr"] = G_lr
                optimizerD.state_dict()["param_groups"][0]["lr"] = D_lr
                ...
                if (iters % ITER_PER_CYCLE <= 100 and iters % 2 == 0) or \
                            (iters % ITER_PER_CYCLE > 100 and iters % 100 == 0): 
                    ...
                    train_bar.set_description(desc="Cycle: [%d|%d]  Iter/Iter_per_cycle: %d/%d, Loss_D:%.6f, Loss_G:%.6f, \
D(x):%.4f, D(G(z)):%.4f, PSNR:%.4f, G_lr:%.4e, D_lr:%.4e" % (cycle, NUM_CYCLES, iters, ITER_PER_CYCLE, 
                ...
                if (iter_count % ITER_PER_CYCLE) % 1000 == 0:
                    img_path = '../datasplit/esemble_train/'
        ...
        with torch.no_grad():
            ...
            SR_path = "../datasplit/test/1/SRGAN_esemble/SR%d" % cycle
        ...
        D_path, G_path = "../datasplit/esemble_train/D_model%d.pkl" % cycle, "../datasplit/esemble_train/G_model%d.pkl" % cycle
        torch.save(D.state_dict(), D_path)  
        torch.save(G.state_dict(), G_path)                 
        print("Take a snapshot of SRGAN in %dth cycle!\n" % cycle)

        # save train_result per cycle
        stat_path = '../SRGAN_esemble_train.csv'
