...
#snapshot ensembles: 5 cycles, T_init=1 & T_mult=2 

Unsatisfactory re-convergence may be due to “Warm restarts did not work originally with Adam due to its dysfunctional weight decay.”

Decoupled weight decay Adam + warm restarts
1 cycle contains multiple epochs
'''
...
if __name__ == "__main__":
    ...
    NUM_EPOCHS = sum(EPOCH_PER_CYCLE)
    ITER_PER_CYCLE = [ITER_PER_EPOCH*ele for ele in EPOCH_PER_CYCLE]    
    ...
    # # G.load_state_dict(torch.load('../datasplit/pretrain/AR-AR/G_model1.pkl'))
    # # D.load_state_dict(torch.load('../datasplit/pretrain/AR-AR/D_model1.pkl'))
    G.load_state_dict(torch.load('../datasplit/esemble_train/train6/G_model4.pkl'))
    D.load_state_dict(torch.load('../datasplit/esemble_train/train6/D_model4.pkl'))       
    ...
    optimizerG, optimizerD = torch.optim.AdamW(G.parameters(), lr=G_lr_init, betas=(0.5, 0.999)), \
                torch.optim.AdamW(D.parameters(), lr=D_lr_init, betas=(0.5, 0.999)) #decoupled weight decay regularization
    
    #to record iter times in the total training
    train_result = {'iter':[], 'd_loss':[], 'g_loss':[], 'psnr':[], 'g_lr':[], 'd_lr':[]}
    iter_count = 15*25569+1

    print("Initializing Snapshot Esembles Training!")
    
    for cycle in range(4, NUM_CYCLES):
        ...
        for epoch in range(1, 6):
            ...
            for x, y in train_bar:
                ...
                G_lr = snapshot_lr(G_lr_init, G_lr_end, iters, ITER_PER_CYCLE[cycle])
                D_lr = snapshot_lr(D_lr_init, D_lr_end, iters, ITER_PER_CYCLE[cycle])        
...
Loss_D:%.6f, Loss_G:%.6f, D(x):%.4f, D(G(z)):%.4f, PSNR:%.4f, G_lr:%.4e, D_lr:%.4e" % ((cycle+1), NUM_CYCLES, 
iters, ITER_PER_CYCLE[cycle], d_loss, g_loss, d_score, g_score, psnr, G_lr, D_lr))

                if (iter_count % ITER_PER_CYCLE[cycle]) % 1000 == 0:
                    img_path = '../datasplit/esemble_train/train6/'
        ...
        SR_path = "../datasplit/test/1/SRGAN_esemble/test6/SR_cycle%d" % (cycle+1)
        ...
        D_path, G_path = "../datasplit/esemble_train/train6/D_model%d.pkl" % (cycle+1), "../datasplit/esemble_train/train6/G_model%d.pkl" % (cycle+1)
        torch.save(D.state_dict(), D_path)  
        torch.save(G.state_dict(), G_path)                 
        print("Take a snapshot of SRGAN in %dth cycle!\n" % (cycle+1))

        # save train_result per cycle
        stat_path = '../datasplit/esemble_train/train6/SRGAN_esemble_train.csv'
