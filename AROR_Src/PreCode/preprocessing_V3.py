...
ARPAM_mat, ORPAM_mat = scipy.io.loadmat('../PAaror/AR_3.mat'), scipy.io.loadmat('../PAaror/OR_3.mat')
...
AR_npy[2] = Denoising1(AR_norm,5,5)
OR_npy[2] = Denoising1(OR_norm,5,5)

export_path1 = '../PAaror/AR_denoise/'+str(3)+'.png'
export_path2 = '../PAaror/OR_denoise/'+str(3)+'.png'
mpimg.imsave(export_path1, AR_npy[2])
mpimg.imsave(export_path2, OR_npy[2])
...
#np.save('../Data_aug/AR3_match.npy', AR3_match) 
#np.save('../Data_aug/OR3_match.npy', OR3_match)


np.save('../AR_npy.npy', AR_npy) 
np.save('../OR_npy.npy', OR_npy)
...
def Gamma_Transform1(img, options): # We choose gamma value 0.6 as the brinary transformation.
    """
    img: ndarray
    gamma: coefficient of gamma transform
    """
...
for i in range(10):
    for j in range(144):
        if i == 2: #only AR/OR_3 with size(1567,1567)
            AR3_aug = Add_Gaussian_Noise(Gamma_Transform1(Elastic_Deformation(Rotation(Flip(AR3_match, 
            ...
            OR3_aug = Add_Gaussian_Noise(Gamma_Transform1(Elastic_Deformation(Rotation(Flip(OR3_match, 
                    parms[j]['flip_option']), parms[j]['rot_option']), parms[j]['ela_parm']['sigma'],
                    parms[j]['ela_parm']['seed']), parms[j]['gamma_option']), parms[j]['noise_option'])
            OR_aug[2,j,0:OR3_aug.shape[0], 0:OR3_aug.shape[1]] = OR3_aug
        else:
            AR_aug[i, j, :, :] = Add_Gaussian_Noise(Gamma_Transform1(Elastic_Deformation(Rotation(Flip(AR_npy[i], 
                          parms[j]['flip_option']), parms[j]['rot_option']), parms[j]['ela_parm']['sigma'],
                           parms[j]['ela_parm']['seed']), parms[j]['gamma_option']), parms[j]['noise_option'])
            OR_aug[i, j, :, :] = Add_Gaussian_Noise(Gamma_Transform1(Elastic_Deformation(Rotation(Flip(OR_npy[i], 
                          parms[j]['flip_option']), parms[j]['rot_option']), parms[j]['ela_parm']['sigma'],
                           parms[j]['ela_parm']['seed']), parms[j]['gamma_option']), parms[j]['noise_option'])
