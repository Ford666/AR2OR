   ...
   for i in range(rows):
       for j in range(cols):
           if (img[i,j] > threshold2):
                ...
                if i-edge<0 or i+edge>=rows or j-edge<0 or j+edge>=cols:
                   None
                else:
                    Temp = img[i-edge:i+edge+1, j-edge:j+edge+1]
                    ncount1 = np.sum(Temp < threshold1) 
                    ncount2 = np.sum(Temp > threshold2)
                    if ncount1 > lentemp/5 : #孤立亮点较边缘点而言，邻域内所含像素值陡低的点要多（一般占1/5以上）
                        img_denoise[i,j] = np.median(Temp)  #中值滤波
                    if img[i,j] > mean+4*std and ncount2 < lentemp*2/3: #斑块噪声不如过曝血管区域的整体像素值都高
                        img_denoise[i,j] = 0
   return img_denoise
...
#AR_npy = np.load('../Data_aug/AR_npyV1.npy') 
#OR_npy = np.load('../Data_aug/OR_npyV1.npy')

## Special processing

## For the 1st pair of AR/OR images, crop the left area of size(2000,1427)
#AR_0, OR_0 = AR_npy[0,0:2000,0:1427].copy(), OR_npy[0,0:2000,0:1427].copy()
#AR_npy[0], OR_npy[0] = 0, 0
#AR_npy[0,0:AR_0.shape[0],0:AR_0.shape[1]], OR_npy[0,0:OR_0.shape[0],0:OR_0.shape[1]] = AR_0, OR_0

## For the 3rd pair, crop the common area of size (1567,1567) 
#AR_match = AR_npy[2,160:1983,100:2000].copy()
#OR_match = OR_npy[2,0:1823,10:1910].copy()
#AR_2, OR_2 = AR_match[323:1823, 0:1500], OR_match[323:1823, 0:1500] #(1500,1500)

#AR_npy[2], OR_npy[2] = 0, 0
#AR_npy[2,0:AR_2.shape[0],0:AR_2.shape[1]], OR_npy[2,0:OR_2.shape[0],0:OR_2.shape[1]] = AR_2, OR_2


#export_path3 = '../PAaror/AR_denoise/'+str(3)+'_new.png'
#export_path4 = '../PAaror/OR_denoise/'+str(3)+'_new.png'
#mpimg.imsave(export_path3, AR_2)
#mpimg.imsave(export_path4, OR_2)


#np.save('../Data_aug/AR_npyV2.npy', AR_npy) 
#np.save('../Data_aug/OR_npyV2.npy', OR_npy)
...
AR_aug = np.zeros(shape=(8,144,2000,2000), dtype=np.float32)
OR_aug = np.zeros(shape=(8,144,2000,2000), dtype=np.float32)
parms, parm = [], {}
for i in range(4):
    for j in range(3):
        for k in range(3):
...
f.write ("Data augmentation done: AR_aug(8,144,2000,2000), OR_aug(8,144,2000,2000)")
f.flush()
f.close()
